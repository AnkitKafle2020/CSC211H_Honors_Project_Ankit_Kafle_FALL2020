...
void studentClass::setUP()
{
    ...
    QSqlDatabase mdb_quizSetup = QSqlDatabase::database(); //calling database
    mdb_quizSetup.open();
    QString nameofquiz =quizName;
    QSqlQuery qry_check(mdb_quizSetup); //initialize the quiz
    int i ;
    ...
    //get question form database and show into ui element of student class
...
//get QuzName and store quizName , we will be using quiz name somewhere to sotre and get info from database
void studentClass::setQuizName(QString name)
{
   quizName = name;
}

// we will be using username to access and store database somewhere in this class
...
void studentClass::on_pushButton_clicked()
{
    ...
    QSqlDatabase mdb_recordGrade = QSqlDatabase::database();
    QSqlQuery qry_check(mdb_recordGrade); //making new database connection to store student name and quiz name
...
//get check box input form ui element
...
qry.bindValue(":ans3",a3); //binding the ans from checkbox, and writing in databse
...
QSqlQuery qry_openStuQuiz(mdb_recordGrade); // Query to open student quiz data
QSqlQuery qry_openQuizAnswer(mdb_recordGrade); // to open teachr quiz tabel
QSqlQuery qry_stuLogininfo(mdb_recordGrade); // open student login info to get userName and Password
...
//write student name , useraname , points and quiz name into database
