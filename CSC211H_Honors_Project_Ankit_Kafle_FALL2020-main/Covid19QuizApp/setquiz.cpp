...
// all the function below functions similarly
void setQuiz::on_pushButton_101_clicked()
{
    ...
    QSqlDatabase mdb_setAns1 = QSqlDatabase::database(); //database connection
    mdb_setAns1.open();
    QSqlQuery qry(mdb_setAns1); //setting up sql query
    QString question,tanswer ="";  //variable to store question form ui until sotirng it to databse
    if(ui->checkBox_111->checkState()) //getting answer from the database
    {
        tanswer = "a";
    }
    ...
    if(mdb_setAns1.open())
    {
       ...
       qry.bindValue(":question",tquestion); //binding the query and value
       qry.bindValue(":answer",tanswer);
       qry.exec();                             // Executing the query
    }
    else
    {
        QMessageBox::information(this,"User Input","Database not open"); //error checing
    }
//changing the color once the bottom is pressed, so we can visually see if the question is set or not
